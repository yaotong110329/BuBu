...
Drivvo 匯入延後到使用者提供實際 Drivvo 匯出檔後再設計。第一版不建立猜測性的 parser、欄位對應或匯入骨架，也不宣稱支援任何尚未驗證的 Drivvo 版本、編碼或格式。

取得樣本後必須另行確認：來源車輛範圍、欄位語意、日期與數字格式、資料缺漏、重複判定、預覽、錯誤報告及 Transaction 邊界。這項延後不影響 CSV 報表匯出與 `.bubu` 完整備份還原。
...
Drivvo 舊資料匯入待取得真實匯出檔後另立規格，不屬於目前第一版承諾。
